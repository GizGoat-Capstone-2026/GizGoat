...
with open(
    METADATA_DIR / "model_metadata.json",
    "r"
) as file:
...
def generate_sleep_recommendation(
    ...
    if score >= 8:
        ...
        return (
            "Sleep quality is excellent. Maintain your current sleep routine."
        )

    elif score >= 6:

        return (
            "Sleep quality is moderate. Consider improving sleep consistency and reducing stress."
        )

    return (
        "Sleep quality is low. Increase sleep duration and improve sleep habits."
    )
...
def predict_sleep(
    ...
    return SleepResponse(
        ...
        sleep_category=get_sleep_category(
            sleep_score
        ),
        recommendation=generate_sleep_recommendation(
            sleep_score
        )
    )
...
@app.post(
    "/foods/tracker",
    response_model=FoodTrackerResponse
)
def food_tracker(
    request: FoodTrackerRequest
):

    result = calculate_food_totals(
        request.foods
    )
